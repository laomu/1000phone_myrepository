...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@include file="./deal/verifylogin.jsp" %>
<!DOCTYPE html>
<html>
	<meta charset="UTF-8">
		...
		<div>
             <div class="container">
                <div class="service-head">
                    <h3 style="color:#ddd;">黑盾网络信息平台会员完善个人资料</h3>
                    <p style="color:#ddd;">这里是技术的殿堂，这里是艺术的海洋，在这里你将享受一场技术的SPA，在这里你将畅游艺术的海洋</p>
                    <%
                    	String erinfo = (String)request.getAttribute("ERROR");
                    	if(erinfo != null) {
                    %>
                    	<p style="color:#f00"><%=erinfo %></p>
                    <%} %>
                </div>
                <div style="height:60px;width:100%"></div>
                <div class="col-md-6">
                    <div class="contact-right wow fadeInLeft" style="color:#fff" data-wow-delay="0.4s">
                        <form method="post" action="${pageContext.request.contextPath }/deal/dealperfectinfo.jsp">
                            姓名：<input type="text" class="text" name="realName" value="<%=member.getRealName() %>" >
                            账号：<input type="text" class="text" name="username" readonly="readonly" value="<%=member.getUsername()%>[不允许修改]" />
                            联系方式：<input type="text" class="text" name="phone" value="<%=member.getPhone()%>">
                            邮箱：<input type="text" class="text" name="email" value="<%=member.getEmail()%>">
                            注册时间：<input type="text" class="text" name="new_pwd2" readonly="readonly" value="<%=member.getRegistTime()%>[不允许修改]">
                            
                            <input class="wow shake" data-wow-delay="0.3s" type="submit" value="提交修改" />
                        </form>
                    </div>
                </div>
            </div>

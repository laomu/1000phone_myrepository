		...
		<form action="" method="">
			...
			<div class="search_add">
				...
				<input type="button" value="增加" class="btn_add"
					onclick="location.href='${pageContext.request.contextPath}/admin/deal/dealvidioadd.jsp'" />
			</div>
			<!--删除等的操作提示-->
			...
			<div id="data">
				<%
				Map<CourseType, List<Course>> map = (Map<CourseType, List<Course>>) request.getAttribute("courseMap");
					Set<Entry<CourseType, List<Course>>> entry = map.entrySet();

					Iterator it = entry.iterator();

					while (it.hasNext()) {
						Entry<CourseType, List<Course>> e = (Entry<CourseType, List<Course>>) it.next();

						CourseType ct = e.getKey();
				%>
				<h3>
					[文档类型]&nbsp;&nbsp;&nbsp;&nbsp;<%=ct.getCtName()%>
				</h3>
				<div class="datainfo">
					<table id="datalist">
						...
						<tr class="datalistinfo">
							...
							<td><img
								src="${pageContext.request.contextPath}/<%=course.getCourseImg() %>"
								style="width: 50px; height: 50px;" /></td>
							<td><%=course.getCourseName()%></td>
							<td><%=course.getManager().getUsername()%></td>
							<td><%=course.getManager().getRealName()%></td>
							<td><%=course.getPublishTime()%></td>
							<td><%=course.getManager().getPhone()%></td>
							<td class="td_modi">

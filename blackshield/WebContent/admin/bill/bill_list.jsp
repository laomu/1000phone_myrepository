...
<script language="javascript" type="text/javascript">
	...
	function deleteAccount() {
		...
		Toolsument.getElementById("operate_result_info").style.display = "block";
	}
	//开通或暂停
	function setState() {
		...
		<form action="" method="">
			...
			<div class="search_add">
				...
				<input type="button" value="增加" class="btn_add"
					onclick="location.href='${pageContext.request.contextPath}/admin/deal/dealbilladd.jsp'" />
			</div>
			<!--删除等的操作提示-->
			...
			<div id="data">
				...
				<div class="datainfo">
					<table id="datalist">
						<%
							List<Tools> dlist = e.getValue();
								if (dlist != null && dlist.size() > 0) {
									for (int i = 0; i < dlist.size(); i++) {
										Tools tools = dlist.get(i);
						%>
						<tr class="datalistinfo">
							<td><%=tools.getToolsId()%></td>
							<td><img
								src="${pageContext.request.contextPath}/<%=tools.getToolsImg()%>"
								style="width: 50px; height: 50px;" /></td>
							<td><%=tools.getTname()%></td>
							<td><%=tools.getManager().getUsername()%></td>
							<td><%=tools.getManager().getRealName()%></td>
							<td><%=tools.getPublishTime()%></td>
							<td><%=tools.getManager().getPhone()%></td>
							<td class="td_modi">
